...
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '../core/shadcn/tooltip'
import { Popover, PopoverContent, PopoverTrigger } from '../core/shadcn/popover'
...
const GoalExpense = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setLoading(true)
    setOpen(false)
    const data = await postData('/limit-expense', values)
    setLoading(false)
    setOpen(true)
  }
  return (
            ...
            <TooltipProvider >
              <Tooltip >
                <TooltipTrigger asChild>
                  <Button
                    variant='ghost' size='icon' className='hover:rounded-full'>
                    <Goal />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
